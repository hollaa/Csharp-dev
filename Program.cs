...
class FirstAssignment {
    ...
    static void BubbleSort(int[] unsortedArray) {
        ...
        for (int i = 0; i < length - 1; i++) {
            ...
            for (int j = 0; j < position; j++) {
                if (unsortedArray[j] > unsortedArray[j + 1]) { //if current element is greater than the next one, swap it forward
    ...
    static void Merge(int[] arr, int left, int mid, int right) {
        ...
        for (i = 0; i < sizeLeft; i++) { //copying the left subarray to the temporary 
            tempLeft[i] = arr[left + i];
        }

        for (j = 0; j < sizeRight; j++) { //copying the right subarray to the temporary 
        ...
        while (i < sizeLeft && j < sizeRight) {
            if (tempLeft[i] <= temptRight[j]) { //comparing each element on each index, 
        ...
        while (i < sizeLeft) { //if there are any elements remaining in the left subarray, copy them to the initial array
            arr[k] = tempLeft[i];
            i++;
            k++;
        }

        while (j < sizeRight) { //if there are any elements remaining in the right subarray, copy them to the initial array
            arr[k] = temptRight[j];
            j++;
            k++;
        }
